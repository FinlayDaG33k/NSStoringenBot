<?php
	class NSStoringenBot{
		// Write stuff to log
		function writeLog($conn, $data){
			$sql = "INSERT INTO `Logs` (`ChatID`,`Date`,`Entry`) VALUES ('".mysqli_real_escape_string($conn,$data["Recipient"])."','".mysqli_real_escape_string($conn,date("Y-m-d h:i:s"))."','".mysqli_real_escape_string($conn,$data["Input"])."');";
			$conn->query($sql);
		}
		// Send a message
		...
		function MySQLInit($sql_servername, $sql_username, $sql_password, $sql_dbname){
		...
		// Parse XML into a nice little array
